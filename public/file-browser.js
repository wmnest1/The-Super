...
(function(){
  ...
  function lbShow(i){
    ...
    const d = lbImgs[lbIdx];
    lbEl.querySelector('.fblb-img').src = '/api/docs/' + d.id + '/view';
    lbEl.querySelector('.fblb-cap').textContent = titleOf(d) + '  \u00B7  ' + (lbIdx + 1) + ' / ' + lbImgs.length;
    [lbIdx - 1, lbIdx + 1].forEach(function(j){
      const n = lbImgs[(j + lbImgs.length) % lbImgs.length];
      if (n){ const im = new Image(); im.src = '/api/docs/' + n.id + '/view'; }
    });
  }
  function lbClose(){ if (lbEl) lbEl.style.display = 'none'; document.removeEventListener('keydown', lbKey); }
  function lbKey(e){
  ...
  function openLightbox(imgs, start){
    ...
    if (!lbEl){
      ...
      lbEl.className = 'fblb';
      lbEl.innerHTML = '<button class="fblb-x" aria-label="Close">\u2715</button>'
        + '<button class="fblb-nav fblb-prev" aria-label="Previous">\u2039</button>'
        + '<img class="fblb-img" alt="">'
        + '<button class="fblb-nav fblb-next" aria-label="Next">\u203A</button>'
        + '<div class="fblb-cap"></div>';
      document.body.appendChild(lbEl);
      lbEl.addEventListener('click', function(e){
        if (e.target.closest('.fblb-prev')) return lbShow(lbIdx - 1);
        if (e.target.closest('.fblb-next')) return lbShow(lbIdx + 1);
        if (e.target.closest('.fblb-x') || e.target === lbEl) return lbClose();
      });
      let tx = null;
      ...
      lbEl.addEventListener('touchend', function(e){
        ...
        if (dx > 40) lbShow(lbIdx - 1); else if (dx < -40) lbShow(lbIdx + 1);
      }, { passive: true });
    }
  ...
  window.renderFileBrowser = function(container, scope, opts){
    ...
    function render(docs){
      ...
      itemsEl.innerHTML = docs.map(d => {
        ...
        const acts = d.sig
          ? ''
          : '<button class="fb-a" data-a="rename" title="Edit title">\u270F\uFE0F</button><button class="fb-a" data-a="del" title="Delete">\u{1F5D1}\uFE0F</button>';
  ...
  + '.fb-btn{background:#0f3460;border:1px solid #2a3a5e;border-radius:7px;color:#cdd;font-size:12px;padding:8px 11px;cursor:pointer;white-space:nowrap}'
  + '.fb-thumb{cursor:pointer;transition:box-shadow .15s ease, transform .15s ease}'
  + '.fb-item .fb-thumb:hover{box-shadow:0 0 0 2px var(--accent,#e8a020);transform:translateY(-1px)}'
  ...
  + '.fblb{position:fixed;inset:0;z-index:120000;background:rgba(0,0,0,.92);display:none;align-items:center;justify-content:center}'
  + '.fblb-img{max-width:92vw;max-height:88vh;object-fit:contain;border-radius:6px;box-shadow:0 10px 40px rgba(0,0,0,.8)}'
  + '.fblb-x{position:absolute;top:14px;right:18px;background:none;border:none;color:#ccc;font-size:26px;cursor:pointer;z-index:2}'
  + '.fblb-x:hover{color:#fff}'
