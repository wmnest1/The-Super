...
function buildSystemPrompt(data, invoiceAccentColor = '#e8a020') {
  ...
  // Time-reference helpers for AI time-aware queries
  const _now = new Date();
  ...
  const endOfLastMonth = new Date(_yr,_mo,0).toISOString().slice(0,10);
  const startOfWeek = new Date(_now.getTime()-_now.getDay()*86400000).toISOString().slice(0,10);
  const past14 = new Date(_now.getTime()-14*86400000).toISOString().slice(0,10);
