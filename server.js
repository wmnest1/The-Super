...
- For out-of-scope hours: if Walt says "out of scope", "extra work", "change order", "additional work", "bill separately", or similar — set outOfScope: true on those crew member entries. When generating invoices for projects that have OOS entries, show regular contract hours and OOS hours as SEPARATE line items (e.g. "Labor — Contract Work" and "Labor — Additional Work (Out of Scope)").
...
PROJECTS:
${(data.projects||[]).map(p => `- ${p.name} [${p.status||"Active"}]${p.client?" | Client: "+p.client:""}${p.address?" | "+p.address:""}${p.startDate?" | Start: "+p.startDate:""}${p.rate?" | Billing: $"+p.rate+"/hr":""}${p.contractAmount?" | Contract: $"+p.contractAmount:""}${(p.markup!==undefined&&p.markup!=="")?" | Materials Markup: "+p.markup+"%":""}${p.notes?" | Scope: "+p.notes:""}`).join("\n") || "None on file"}
...
${(data.entries||[]).length === 0 ? "No entries logged yet" : [...(data.entries||[])].sort((a,b)=>(b.date||"").localeCompare(a.date||"")).map(e => {
  const crew = (e.crew||[]).map(c=>`${c.name} ${c.hours}hr${c.outOfScope?' [OOS]':''}`).join(", ");
        ...
        crew: {
          ...
          items: {
            ...
            properties: {
              ...
              outOfScope: { type: "boolean", description: "Set true if Walt says these hours are out of scope, extra work, change order, additional work, or should be billed separately from the base contract." }
            },
            required: ["name", "hours"]
          }
        },
  ...
  switch (toolName) {
    case "save_entry": {
      ...
      const entry = {
        ...
        crew: (input.crew || []).map(c => ({ name: c.name, hours: c.hours, ...(c.outOfScope ? { outOfScope: true } : {}) })),
