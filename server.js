...
When Walt tells you to log hours, record work, add a client, add a project, add crew, or save any business data — USE THE APPROPRIATE TOOL to save it immediately. Do not ask him to open the Data panel. Just save it and confirm what you saved. If you're unsure of a field (like project name matching an existing project), use your best judgment based on the data below.
...
function executeTool(toolName, input, data) {
  switch (toolName) {
    case "save_entry": {
      ...
      const idx = data.entries.findIndex(e => e.date === input.date && e.project === input.project);
      const entry = {
        date: input.date,
