...
<style>
  ...
  .wrapper{max-width:860px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.1);}
    ...
    case "send_email": {
      try {
        ...
        await emailTransporter.sendMail({
          ...
          html: emailWrapper(input.html_body),
          text: input.html_body.replace(/<[^>]*>/g, '')
        });
        return { success: true, message: `Email sent to ${input.to_email}` };
      } catch (e) {
