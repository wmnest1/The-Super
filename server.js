...
app.get("/api/docs", async (req, res) => {
  try {
    ...
    res.json(docs.map(d => ({ id: d._id.toString(), project: d.project, client: d.client, name: d.name, docType: d.docType, kind: d.kind, mimeType: d.mimeType, size: d.size, uploadedAt: d.uploadedAt, source: d.source, meta: d.meta || {}, title: (d.meta && d.meta.title) || '' })));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
