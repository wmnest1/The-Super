...
async function generatePDF(html) {
  ...
  let execPath;
  try {
    execPath = execSync('which chromium-browser || which chromium || which google-chrome || which google-chrome-stable')
      .toString().trim();
  } catch(e) {
    const paths = [
  ...
  if (!execPath) throw new Error('No Chrome/Chromium found on system');
